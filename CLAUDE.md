# Brownfield Agent — helppilot

**Run ID:** n979knp3sewrb83brmts2wmmk583sx6d
**Project ID:** jd74zk3zp3j5ncpv53c70h112n83ptss
**Repo:** /Users/damonbodine/Factory512/builds/helppilot-20260326-223058/helppilot
**Mode:** feature
**Request:** Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Auto-Triage AI — when new ticket submitted, AI reads title + description and suggests: category, priority, tags, and which agent should handle it. Show as suggestion bar at top of ticket that agent can accept/modify. 2) Resolution Suggester — when agent opens ticket, AI searches knowledge base and past resolved tickets with similar content, returns top 3 suggested solutions with relevance scores. 3) Auto-Response Drafter — draft initial response to requester. For common issues provide solution steps. For complex ones acknowledge receipt and set expectations. 4) Knowledge Article Generator — after resolving a ticket, AI generates draft KB article from the ticket conversation: problem, symptoms, root cause, solution steps, prevention tips. Each feature needs: Convex action, React component, integration into existing pages.
...
5. Work ONLY inside the target repository at /Users/damonbodine/Factory512/builds/helppilot-20260326-223058/helppilot. Never create files outside it.
...
**Query ready steps:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getReadySteps '{"runId":"n979knp3sewrb83brmts2wmmk583sx6d"}'
```

**Get run status (for resume):**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getRunStatus '{"runId":"n979knp3sewrb83brmts2wmmk583sx6d"}'
```

**Load artifacts by kind:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/queries:getArtifactsByKinds '{"runId":"n979knp3sewrb83brmts2wmmk583sx6d","kinds":["kind1","kind2"]}'
```

**Push artifact (marks step completed):**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/cliComplete:completeStepWithArtifact '{"runId":"n979knp3sewrb83brmts2wmmk583sx6d","stepNumber":N,"artifactKind":"kind","artifactContent":"<json-string>","contentType":"json"}'
```

**Complete step with no artifact:**
```
npx convex run --url https://friendly-elk-878.convex.cloud brownfield/cliComplete:completeStepNoArtifact '{"runId":"n979knp3sewrb83brmts2wmmk583sx6d","stepNumber":N}'
```
...
### INITIALIZE
1. `cd` to `/Users/damonbodine/Factory512/builds/helppilot-20260326-223058/helppilot`.
...
Parse `Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Auto-Triage AI — when new ticket submitted, AI reads title + description and suggests: category, priority, tags, and which agent should handle it. Show as suggestion bar at top of ticket that agent can accept/modify. 2) Resolution Suggester — when agent opens ticket, AI searches knowledge base and past resolved tickets with similar content, returns top 3 suggested solutions with relevance scores. 3) Auto-Response Drafter — draft initial response to requester. For common issues provide solution steps. For complex ones acknowledge receipt and set expectations. 4) Knowledge Article Generator — after resolving a ticket, AI generates draft KB article from the ticket conversation: problem, symptoms, root cause, solution steps, prevention tips. Each feature needs: Convex action, React component, integration into existing pages.` against the `codebase_profile`.
...
```json
{
  "runId": "n979knp3sewrb83brmts2wmmk583sx6d",
  "projectName": "helppilot",
  "mode": "feature",
  "request": "Add 4 AI features using OpenRouter API (OPENROUTER_API_KEY env var, model: nvidia/nemotron-3-super-120b-a12b:free). Create convex/ai.ts with a reusable action pattern. Features: 1) Auto-Triage AI — when new ticket submitted, AI reads title + description and suggests: category, priority, tags, and which agent should handle it. Show as suggestion bar at top of ticket that agent can accept/modify. 2) Resolution Suggester — when agent opens ticket, AI searches knowledge base and past resolved tickets with similar content, returns top 3 suggested solutions with relevance scores. 3) Auto-Response Drafter — draft initial response to requester. For common issues provide solution steps. For complex ones acknowledge receipt and set expectations. 4) Knowledge Article Generator — after resolving a ticket, AI generates draft KB article from the ticket conversation: problem, symptoms, root cause, solution steps, prevention tips. Each feature needs: Convex action, React component, integration into existing pages.",
  "summary": "...",
  "stepsCompleted": 0,
...
1. `cd /Users/damonbodine/Factory512/builds/helppilot-20260326-223058/helppilot`
